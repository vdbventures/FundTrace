...
## Deferred Findings — Not Yet Built

### Vanguard US Issuer Feed API (US Step 2 task)

Discovered during Vanguard Canada investigation. The Vanguard US investor site exposes
a clean, unauthenticated REST API for ETF stock holdings:

```
GET https://investor.vanguard.com/investment-products/etfs/profile/api/{TICKER}/portfolio-holding/stock
```

- Returns JSON, no auth headers needed
- Confirmed working for: VOO, VTI (likely all US-listed Vanguard ETFs)
- Response shape: `{ fund: { entity: [ { ticker, longName, percentWeight, cusip, sedol, isin, ... } ] } }`
- `percentWeight` is the allocation in percent (e.g. 7.5775 = 7.58%)
- Pagination: `?page=1&pageSize=25` — iterate until `entity` is empty
- This is the source for the VFV → VOO cross-source reconciliation test

**Do not build yet** — this is US Step 2 (issuer feeds), a separate task from the Canadian ETF adapters.
When that task starts, this URL pattern is the right starting point for Vanguard US.

---

### Canadian ETF Issuer Data Access — Probe Results (2026-05-29)

Probed the top-tier and second-tier Canadian ETF issuers for server-accessible holdings data.
Summary of what works and what doesn't, so the next session doesn't re-probe.

**Needs browser automation (defer to architectural decision):**
- **Vanguard Canada** — Angular Universal SSR. Individual holdings NOT in server HTML
  (NVIDIA absent from 209KB VFV page). GraphQL at `/api/graphql` is browser-only proxy.
  Option B server-side probe found no accessible REST endpoint after exhaustive guessing.
  Fund ID 9563 = VFV. Licensing: robots.txt permissive; ToS page 404.
- **iShares Canada (BlackRock)** — Old `.ajax` endpoint (timestamp `1467271812596`) returns
  HTML, not CSV. New endpoint unknown. Page source references `blackrock.com/api-gateway`
  but no working public endpoints found.
- **BMO ETFs** — Correct data domain not found; all guessed domains returned 404.
- **Purpose Investments (equity ETFs)** — Portfolio pages render security names via JS only;
  server-rendered HTML shows sector/weight metadata but NOT company names.
  `/api/metro?historical_holdings=BTCC` returns CSV of BTC quantities only (not equity holdings).
- **Mackenzie Investments** — Returns HTTP 403 on all product page URLs.

**Partially accessible (top N only, no complete list):**
- **Global X Canada (Horizons)** — Top 10 holdings present as server-rendered HTML on each
  product page at `globalx.ca/products/detail/{TICKER}`. Security names ARE shown (e.g.
  Royal Bank of Canada 7.04%). No CSV/JSON download, no full list, no ISIN/CUSIP.
  Sitemap has no dedicated holdings pages. Swap-based ETFs (HXS, HXT) hold swap
  agreements, not underlying stocks — look-through not meaningful for those.
- **CI Global Asset Management** — Top 15 holdings accessible via unauthenticated AJAX:
  ```
  GET https://funds.cifinancial.com/en/ajax/get_fund_chunk.html
      ?series_id={SERIES_ID}&chunk=portfolio&type=3&classId={CLASS_ID}&currencySelector=1
  ```
  Returns an HTML fragment. Series_id and classId are embedded in product page URLs.
  Known series IDs: CEQT=14328/classId=455, CUSM=14194/classId=455, CCDN=14195/classId=456,
  CASH=14184/classId=455. No ISIN/CUSIP in the fragment; no ticker symbols.
  For fund-of-funds (CEQT, CGRO, CBAL) the complete underlying-ETF allocation IS returned
  (e.g. CEQT = CUSM.B 38.82%, CCDN 28.95%, IEFA 22.04%, IEMG 10.18% — 99.99% covered).
  For equity index ETFs (CUSM, CCDN) only top 15 individual stocks are shown.

**Architectural decision needed before building adapters for the "defer" group above.**
CI fund-of-funds and Global X top-10 can be built now with a clear partial-data disclaimer.
Complete-list access for any Canadian ETF issuer not yet found without browser automation.
